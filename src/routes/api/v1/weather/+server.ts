...
async function fetchWeather() {
  ...
  const weather: WeatherData = await fetch(
    `${WEATHER_API}/forecast?${WEATHER_LAT_LONG}&current=apparent_temperature,temperature_2m,wind_speed_10m,wind_direction_10m,wind_gusts_10m,cloud_cover,is_day,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph&precipitation_unit=inch&daily=sunrise,sunset&timezone=America%2FNew_York`,
    requestOptions
  )
